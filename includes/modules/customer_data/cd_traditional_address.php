...
{
    public const CONFIG_KEY_BASE = 'MODULE_CUSTOMER_DATA_TRADITIONAL_ADDRESS_';

    public const PROVIDES = [ 'address' ];
    public const REQUIRES = [ 'name', 'street_address', 'postcode', 'city', 'country' ];
    public const FIELDS = [ 'name', 'street_address', 'postcode', 'city', 'country_id', 'company', 'suburb', 'state' ];

    private ?array $active_fields = null;
    ...
    public function process(&$customer_details): bool
    {
    ...
    public function get_purveyors(): array
    {
    ...
    public function format(array $address, $html, string $boln, $eoln)
    {
        ...
        htmlspecialchars($GLOBALS['customer_data']->get('name', $address) ?? '');

        $street = htmlspecialchars((string) $address['street_address']);
        ...
        if (!empty($address['country_id'])) {
            ...
            $country = htmlspecialchars((string) $address['country']['title']);
        } else {
            $country = '';
        }
        ...
        if (!empty($company)) {
            return $company . $cr . $address;
        }

        return $address;
    }

    public function get_address_format_id($country_id)
    {
