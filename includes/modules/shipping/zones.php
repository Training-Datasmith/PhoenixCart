...
{
    public const CONFIG_KEY_BASE = 'MODULE_SHIPPING_ZONES_';

    // CUSTOMIZE THIS SETTING FOR THE NUMBER OF ZONES NEEDED
    public const ZONE_COUNT = 1;

    protected $destination_zone = false;

    public function update_status_by($address): void
    {
        ...
        for ($i = 1; $i <= static::ZONE_COUNT; $i++) {
            if (in_array($address['country']['iso_code_2'], explode(';', (string) $this->base_constant("COUNTRIES_$i")))) {
    ...
    public function quote($method = '')
    {
        ...
        if (false !== $this->destination_zone) {
            $zones_table = preg_split('{[:,]}', (string) $this->base_constant("COST_{$this->destination_zone}"));
            for ($i = 0, $size = count($zones_table); $i < $size; $i += 2) {
                if ($shipping_weight <= $zones_table[$i]) {
                    $this->quotes['methods'][] = [
                      'id' => $this->code,
                      'title' => sprintf(
                          MODULE_SHIPPING_ZONES_TEXT_WAY,
            ...
            if (!isset($this->quotes['methods'][0])) {
                error_log(sprintf(
                    'Weight [%d] larger than maximum in table [%s] for [%s].',
                    $shipping_weight,
