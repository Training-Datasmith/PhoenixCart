  ...
  <div class="row">
    <?php
        while ($testimonials = $testimonials_query->fetch_assoc()) {
            ...
            echo nl2br((string) $testimonials['testimonials_text']) . PHP_EOL;
            echo '</blockquote>' . PHP_EOL;
            echo '<figcaption class="blockquote-footer pt-2">',
            sprintf(MODULE_CONTENT_TESTIMONIALS_LIST_WRITERS_NAME_DATE, htmlspecialchars((string) $testimonials['customers_name']), Date::abridge($testimonials['date_added'])),
            '</figcaption>' . PHP_EOL;
            echo '</figure>';
            echo '</div>' . PHP_EOL;
        }
