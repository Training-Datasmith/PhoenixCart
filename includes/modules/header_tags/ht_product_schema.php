    ...
    public function execute(): void
    {
        ...
        if (isset($product) && ($product instanceof Product) && $product->get('status')) {
            ...
            $schema_product = [
              ...
              'name'        => htmlspecialchars((string) $product->get('name')),
              ...
              'description' => substr(trim((string) preg_replace('/\s\s+/', ' ', strip_tags((string) $product->get('description')))), 0, 197) . '...',
            ];
            ...
            if (($product->get('manufacturers_id') ?? 0) > 0) {
                $schema_product['manufacturer'] = [
                  '@type' => 'Organization',
                  'name'  => htmlspecialchars((string) $product->get('brand')->getData('manufacturers_name')),
                ];
            ...
            if (count($product->get('reviews')) > 0) {
                $schema_product['aggregateRating'] = [
                  ...
                  'reviewCount' => count($product->get('reviews')),
                ];
                ...
                foreach ($product->get('reviews') as $review) {
                    $schema_product['review'][] = [
                      '@type'         => 'Review',
                      'datePublished' => htmlspecialchars((string) $review['date_added']),
                      'description'   => htmlspecialchars((string) $review['text']),
                      'name'          => htmlspecialchars((string) $product->get('name')),
                      'author'  => [
                        '@type' => 'Person',
                        'name'  => htmlspecialchars((string) $review['customers_name']),
                      ],
                      'reviewRating'  => [
                        '@type'       => 'Rating',
                        'bestRating'  => '5',
