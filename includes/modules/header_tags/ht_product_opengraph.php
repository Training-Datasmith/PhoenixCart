...
{
    public const CONFIG_KEY_BASE = 'MODULE_HEADER_TAGS_PRODUCT_OPENGRAPH_';
    ...
    public function execute(): void
    {
        ...
        if (isset($product) && ($product instanceof Product) && $product->get('status')) {
            ...
            $product_description = substr(trim((string) preg_replace('/\s\s+/', ' ', strip_tags((string) $product->get('description')))), 0, 197) . '...';
            ...
            foreach ($data as $property => $content) {
                $result .= '<meta property="' . htmlspecialchars($property) . '" content="' . htmlspecialchars((string) $content) . '">' . PHP_EOL;
            }
