  ...
  <div class="d-none d-sm-block">
    <div class="d-flex flex-wrap">
      ...
      foreach ($i_brand_array as $i => $o) {
          echo '<a class="list-group-item border-0 p-2" href="' . $GLOBALS['Linker']->build('index.php', ['manufacturers_id' => (int)$o['manufacturers_id']]) . '">' . new Image('images/' . $o['manufacturers_image'], [], htmlspecialchars((string) $o['manufacturers_name'])) . '</a>';
      }
?>
    </div>
  </div>
  <div class="d-block d-sm-none">
    <div id="iCarousel" class="carousel slide" data-bs-ride="carousel">
      <div class="carousel-inner">
        <?php
  $n = 1;
foreach ($i_brand_xs_array as $o) {
    $active = ($n == 1) ? ' active' : null;
    echo '<div class="carousel-item' . $active . '">';
    echo '<div class="list-group list-group-horizontal justify-content-center">';
    foreach ($o as $item) {
        echo '<a class="list-group-item border-0" href="' . $GLOBALS['Linker']->build('index.php', ['manufacturers_id' => (int)$item['manufacturers_id']]) . '">' . new Image('images/' . $item['manufacturers_image'], [], htmlspecialchars((string) $item['manufacturers_name'])) . '</a>';
    }
    echo '</div>';
    echo '</div>';
}
