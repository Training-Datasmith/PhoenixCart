...
{
    public const CONFIG_KEY_BASE = 'I_BRAND_ICONS_';
    /**
     * @var string
     */
    ...
    {
        return [$this->config_key_base . 'STATUS' => ['title' => 'Enable Brand Icons Module', 'value' => 'True', 'desc' => 'Should this module be shown on the index page?', 'set_func' => "Config::select_one(['True', 'False'], "], $this->config_key_base . 'GROUP' => ['title' => 'Module Display', 'value' => 'A', 'desc' => 'Where should this module display on the index page?', 'set_func' => "Config::select_one(['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I'], "], $this->config_key_base . 'CONTENT_WIDTH' => ['title' => 'Content Width', 'value' => 'col-sm-4 mb-4', 'desc' => 'What width container should the content be shown in?'], $this->config_key_base . 'CSV' => ['title' => 'Brands', 'value' => '', 'desc' => 'Display these Brands.', 'set_func' => 'i_select_brands(', 'use_func' => 'i_show_brands'], $this->config_key_base . 'XS_CHUNK' => ['title' => 'Chunk', 'value' => '2', 'desc' => 'At SM and below, the display will change to a Carousel.  This number determines how many icons are in each slide.'], $this->config_key_base . 'SORT_ORDER' => ['title' => 'Sort Order', 'value' => '87', 'desc' => 'Sort order of display. Lowest is displayed first.']];
    }
}
...
function i_get_manufacturers($manufacturers = []): array
{
    return array_merge(is_array($manufacturers) ? $manufacturers : [], $GLOBALS['db']->fetch_all('SELECT manufacturers_id AS id, manufacturers_name AS text FROM manufacturers ORDER BY manufacturers_name'));
}
function i_show_brands($text): string
{
    return nl2br(implode("\n", array_map(i_get_brand_name(...), explode(';', (string) $text))));
}
function i_select_brands($values, string $key): string
{
    $values_array = explode(';', $values);
    $output = '';
    foreach (i_get_manufacturers() as $brand) {
        $output .= '<br>' . (new Tickable('i_selected_brands[]', ['value' => $brand['id']], 'checkbox'))->tick(in_array($brand['id'], $values_array)) . '&nbsp;' . Text::output($brand['text']);
    }
