...
class stripe_sca extends abstract_payment_module
{
    ...
    public function pre_confirmation_check(): void
    {
        if (MODULE_PAYMENT_STRIPE_SCA_CARD_DATA_ONE_LINE == 'True') {
            $GLOBALS['Template']->add_block($this->get_submit_card_details_oneline_javascript(), 'footer_scripts');
        } else {
            $GLOBALS['Template']->add_block($this->get_submit_card_details_multiline_javascript(), 'footer_scripts');
        }
    }
    public function confirmation(): array
    {
        ...
        \Stripe\Stripe::set_api_key($secret_key);
        \Stripe\Stripe::set_api_version($this->api_version);
        $metadata = ['customer_id' => Text::output($_SESSION['customer_id']), 'order_id' => Text::output($order_id), 'company' => isset($order->customer['company']) ? Text::output($order->customer['company']) : ''];
        $content = '';
        ...
        $address = ['address_line1' => $GLOBALS['customer_data']->get('street_address', $order->billing), 'address_city' => $GLOBALS['customer_data']->get('city', $order->billing), 'address_zip' => $GLOBALS['customer_data']->get('postcode', $order->billing), 'address_state' => $GLOBALS['customer_data']->get('state', $order->billing), 'address_country' => $GLOBALS['customer_data']->get('country_iso_code_2', $order->billing)];
        ...
        if (isset($_SESSION['stripe_payment_intent_id'])) {
            try {
                $this->intent = \Stripe\Payment_Intent::retrieve(['id' => $_SESSION['stripe_payment_intent_id']]);
                ...
                $this->event_log($_SESSION['customer_id'], 'page create intent', $_SESSION['stripe_payment_intent_id'], $err->get_message());
                // failed to save existing intent, so create new one
                unset($_SESSION['stripe_payment_intent_id']);
            }
        }
        if (!isset($_SESSION['stripe_payment_intent_id'])) {
            $params = ['amount' => $this->format_raw($order->info['total']), 'currency' => $currency, 'receipt_email' => $customer_mail, 'setup_future_usage' => 'off_session', 'capture_method' => $capture_method, 'metadata' => $metadata];
            $this->intent = \Stripe\Payment_Intent::create($params);
        ...
        $content .= '<input type="hidden" id="intent_id" value="' . Text::output($_SESSION['stripe_payment_intent_id']) . '">' . '<input type="hidden" id="secret" value="' . Text::output($this->intent->client_secret) . '">';
    ...
    public function get_error(): array
    {
        ...
        return ['title' => MODULE_PAYMENT_STRIPE_SCA_ERROR_TITLE, 'error' => $message];
    }
    public function event_log($customer_id, $action, $request, $response): void
    {
        ...
        if (MODULE_PAYMENT_STRIPE_SCA_LOG == 'True') {
            ...
            INSERT into stripe_event_log (customer_id, action, request, response, date_added)
              VALUES ('%s', '%s', '%s', '%s', now())
            EOSQL, (int) $customer_id, $action, $db->escape($request), $db->escape($response)));
        }
    }
    public function get_parameters(): array
    {
        ...
        return [$this->config_key_base . 'STATUS' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_STATUS_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_STATUS_DESC, 'value' => 'True', 'set_func' => "Config::select_one(['True', 'False'], "], $this->config_key_base . 'TRANSACTION_SERVER' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_SERVER_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_SERVER_DESC, 'value' => 'Live', 'set_func' => "Config::select_one(['Live', 'Test'], "], $this->config_key_base . 'LIVE_PUBLISHABLE_KEY' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_LIVE_PUB_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_LIVE_PUB_DESC, 'value' => ''], $this->config_key_base . 'LIVE_SECRET_KEY' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_LIVE_SECRET_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_LIVE_SECRET_DESC, 'value' => ''], $this->config_key_base . 'LIVE_WEBHOOK_SECRET' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_LIVE_WEBHOOK_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_LIVE_WEBHOOK_DESC, 'value' => ''], $this->config_key_base . 'TEST_PUBLISHABLE_KEY' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_TEST_PUB_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_TEST_PUB_DESC, 'value' => ''], $this->config_key_base . 'TEST_SECRET_KEY' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_TEST_SECRET_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_TEST_SECRET_DESC, 'value' => ''], $this->config_key_base . 'TEST_WEBHOOK_SECRET' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_TEST_WEBHOOK_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_TEST_WEBHOOK_DESC, 'value' => ''], $this->config_key_base . 'TOKENS' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_TOKENS_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_TOKENS_DESC, 'value' => 'False', 'set_func' => "Config::select_one(['True', 'False'], "], $this->config_key_base . 'CARD_DATA_ONE_LINE' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_CARD_DATA_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_CARD_DATA_DESC, 'value' => 'False', 'set_func' => "Config::select_one(['True', 'False'], "], $this->config_key_base . 'LOG' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_LOG_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_LOG_DESC, 'value' => 'False', 'set_func' => "Config::select_one(['True', 'False'], "], $this->config_key_base . 'TRANSACTION_METHOD' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_METHOD_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_METHOD_DESC, 'value' => 'Capture', 'set_func' => "Config::select_one(['Authorize', 'Capture'], "], $this->config_key_base . 'PREPARE_ORDER_STATUS_ID' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_NEW_ORDER_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_NEW_ORDER_DESC, 'value' => self::ensure_order_status('MODULE_PAYMENT_STRIPE_SCA_PREPARE_ORDER_STATUS_ID', 'Preparing [Stripe SCA]'), 'set_func' => 'Config::select_order_status(', 'use_func' => 'order_status::fetch_name'], $this->config_key_base . 'ORDER_STATUS_ID' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_PROCESSED_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_PROCESSED_DESC, 'value' => '0', 'set_func' => 'Config::select_order_status(', 'use_func' => 'order_status::fetch_name'], $this->config_key_base . 'TRANSACTION_ORDER_STATUS_ID' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_TRANSACTION_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_TRANSACTION_DESC, 'value' => self::ensure_order_status('MODULE_PAYMENT_STRIPE_SCA_TRANSACTION_ORDER_STATUS_ID', 'Stripe SCA [Transactions]'), 'set_func' => 'Config::select_order_status(', 'use_func' => 'order_status::fetch_name'], $this->config_key_base . 'ZONE' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_ZONE_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_ZONE_DESC, 'value' => '0', 'use_func' => 'geo_zone::fetch_name', 'set_func' => 'Config::select_geo_zone('], $this->config_key_base . 'VERIFY_SSL' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_SSL_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_SSL_DESC, 'value' => 'True', 'set_func' => "Config::select_one(['True', 'False'], "], $this->config_key_base . 'PROXY' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_PROXY_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_PROXY_DESC], $this->config_key_base . 'DEBUG_EMAIL' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_EMAIL_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_EMAIL_DESC], $this->config_key_base . 'DAYS_DELETE' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_DAYS_DELETE_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_DAYS_DELETE_DESC, 'value' => '2'], $this->config_key_base . 'SORT_ORDER' => ['title' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_SORT_TITLE, 'desc' => MODULE_PAYMENT_STRIPE_SCA_ADMIN_SOR_DESC, 'value' => '0']];
    }
    public function send_transaction_to_gateway($url, $parameters = null, $curl_opts = []): bool|string
    {
        ...
        $header = ['Stripe-Version: ' . $this->api_version, 'User-Agent: Phoenix ' . Versions::get('Phoenix')];
    ...
    public function get_test_link_info(): string
    {
        ...
        EOD;
        return '<p><i class="fas fa-lock"></i>&nbsp;<a href="javascript:openTestConnectionDialog();" style="text-decoration: underline; font-weight: bold;">' . MODULE_PAYMENT_STRIPE_SCA_DIALOG_CONNECTION_LINK_TITLE . '</a></p>' . '<div id="testConnectionDialog" style="display: none;"><p>Server:<br>https://api.stripe.com/v1/balance</p><div id="testConnectionDialogProgress"><p>' . MODULE_PAYMENT_STRIPE_SCA_DIALOG_CONNECTION_GENERAL_TEXT . '</p><div id="tcdprogressbar"></div></div></div>' . $js;
    }
    public function get_test_connection_result(): int
    {
        $stripe_result = json_decode((string) $this->send_transaction_to_gateway('https://api.stripe.com/v1/charges/phoenixcart_connection_test'), true);
    ...
    public function get_submit_card_details_multiline_javascript($intent = null): string
    {
    ...
    public function get_submit_card_details_oneline_javascript($intent = null): string
    {
        $stripe_publishable_key = MODULE_PAYMENT_STRIPE_SCA_TRANSACTION_SERVER == 'Live' ? MODULE_PAYMENT_STRIPE_SCA_LIVE_PUBLISHABLE_KEY : MODULE_PAYMENT_STRIPE_SCA_TEST_PUBLISHABLE_KEY;
        $intent_url = $GLOBALS['Linker']->build('ext/modules/payment/stripe_sca/payment_intent.php', '', 'SSL', false, false);
        $js = <<<EOD
    ...
    public function delete_card(string $card, string $customer, $token_id): bool
    {
        global $db;
        $secret_key = MODULE_PAYMENT_STRIPE_SCA_TRANSACTION_SERVER == 'Live' ? MODULE_PAYMENT_STRIPE_SCA_LIVE_SECRET_KEY : MODULE_PAYMENT_STRIPE_SCA_TEST_SECRET_KEY;
        \Stripe\Stripe::set_api_key($secret_key);
        \Stripe\Stripe::set_api_version($this->api_version);
        $error = '';
        $payment_method = \Stripe\Payment_Method::retrieve($card);
        try {
        ...
        if (!isset($result->object) || $result->object !== 'payment_method') {
            $this->send_debug_email($result . PHP_EOL . $error);
        }
        $db->query(sprintf(<<<'EOSQL'
        DELETE
          FROM customers_stripe_tokens
          WHERE id = %s
            AND customers_id = %s
            AND stripe_token = '%s'
        EOSQL, (int) $token_id, (int) $_SESSION['customer_id'], $db->escape(Text::prepare($customer . ':|:' . $card))));
