...
class paypal_api
{
    ...
    public function make_call($url, $parameters = null, $headers = null, $opts = null): bool|string|array
    {
        ...
        if ($this->verifyssl == 'True') {
            ...
            if (str_ends_with($server['host'], 'paypal.com') && file_exists(DIR_FS_CATALOG . 'ext/modules/payment/paypal/paypal.com.crt')) {
        ...
        if (isset($opts['returnFull']) && $opts['returnFull'] === true) {
            $result = ['response' => $result, 'error' => curl_error($curl), 'info' => curl_getinfo($curl)];
        }
        curl_close($curl);
        return $result;
    }
