...
], fn ($k) => (constant($k) > 0) || ($k === $default_column), ARRAY_FILTER_USE_KEY);

uksort($column_specifications, fn ($a, $b) => constant($a) <=> constant($b));
...
if ((isset($_GET['sort'])) && (preg_match('{\A[1-9]\d*[ad]\z}', (string) $_GET['sort'])) && (substr((string) $_GET['sort'], 0, -1) <= count($column_list))) {
    $sort_column = (int)(substr((string) $_GET['sort'], 0, -1)) - 1;
} else {
...
$direction = (str_ends_with((string) $_GET['sort'], 'd')) ? ' DESC' : '';
