...
EOSQL;
     ...
     * @return mixed[]
     */
    protected function extract_address($prefix): array
    {
        $length = strlen((string) $prefix);

        $address = [];
        foreach ($this->data as $k => $v) {
            if (substr((string) $k, 0, $length) === $prefix) {
                $address[substr((string) $k, $length)] = $v;
            }
        }
    ...
    public function build_info(): void
    {
    ...
    public function build_addresses(): void
    {
        $this->order->customer = $this->extract_address('customers_');

        $this->order->delivery = $this->extract_address('delivery_');
        if (('' === trim((string) $this->order->delivery['name'])) && empty($this->order->delivery['street_address'])) {
    ...
    public function build_totals(): void
    {
        ...
        while ($total = $totals_query->fetch_assoc()) {
            ...
            switch ($total['class']) {
                case 'ot_total':
                    $this->order->info['total'] = strip_tags((string) $total['text']);
                    break;
                case 'ot_shipping':
                    $this->order->info['shipping_method'] = trim(rtrim(strip_tags((string) $total['title']), ': '));
                    break;
            }
        }
    }

    public function build_products(): void
    {
    ...
    public static function build(&$order): void
    {
