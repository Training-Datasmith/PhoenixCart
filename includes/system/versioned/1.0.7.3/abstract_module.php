...
{
    public const CONFIG_KEY_BASE = self::CONFIG_KEY_BASE;
    /**
     * @var class-string<\abstract_module>
     */
    public $code;
    public $title;
    public $description;
    ...
    {
        return static::get_constant($this->config_key_base . "{$suffix}");
    }
    ...
    protected function _install($parameters)
    {
        ...
        foreach ($parameters as $key => $data) {
            $sql_data = ['configuration_title' => $data['title'], 'configuration_key' => $key, 'configuration_value' => $data['value'] ?? '', 'configuration_description' => $data['desc'], 'configuration_group_id' => 6, 'sort_order' => $sort_order, 'date_added' => 'NOW()'];
    ...
    public function install($parameter_key = null): void
    {
        ...
        if (isset($parameter_key)) {
            $parameters = isset($parameters[$parameter_key]) ? [$parameter_key => $parameters[$parameter_key]] : [];
        }
