...
class customer_write extends customer_query
{
    ...
    public const IDENTIFIER_SUFFIX = '_id';

    public static function create(array $db_tables, array &$customer_details = []): void
    {
    ...
    public static function update($db_tables, $criteria = []): void
    {
        $foreign_keys = self::FOREIGN_KEYS;
        $parameters = [
          'db' => &$db_tables,
        ...
        $db_tables = array_map(fn ($value) => array_filter($value, fn ($v) => isset($v)), $db_tables);
