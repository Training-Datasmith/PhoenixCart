...
class customer
{
    ...
    public function preload_columns(&$to): void
    {
    ...
    protected function fetch_address(int $to = 0)
    {
        ...
        if ($to > 0) {
            $address_query = $GLOBALS['db']->query($customer_data->build_read(['id', 'address'], 'address_book', ['id' => (int)$this->id, 'address_book_id' => $to]));
        } else {
            $address_query = $GLOBALS['db']->query($customer_data->build_read($customer_data->list_all_capabilities(), 'both', ['id' => (int)$this->id]));
        }

        $this->data[$to] = array_filter($address_query->fetch_assoc(), fn ($v) => !Text::is_empty($v));
    ...
    public function set($key, $value, $to = 0): void
    {
    ...
    public function persist($to = 0): void
    {
        if ($to > 0) {
