...
class hooks
{
    ...
    {
        $this->_site = basename((string) $site);
    ...
    protected function load($group)
    {
        ...
        EOSQL, $GLOBALS['db']->escape($this->_site), $GLOBALS['db']->escape($group)));
        while ($hook = $hooks_query->fetch_assoc()) {
            $callback = $this->build_callback($hook['hooks_class'], $hook['hooks_method']);
            if (is_callable($callback)) {
                Guarantor::guarantee_all($this->_hooks, $this->_site, $hook['hooks_action'])[$hook['hooks_code']] = $callback;
            }
        }
        $this->sort_hooks();
    }
            ...
            if ('php' === $pathinfo['extension']) {
                ...
                foreach (get_class_methods(Guarantor::ensure_global($class)) as $method) {
                    if (substr($method, 0, $this->prefix_length) === self::PREFIX) {
                        ...
                        Guarantor::guarantee_all($this->_hooks, $this->_site, $action)[$pathinfo['filename']] = [$GLOBALS[$class], $method];
                    }
                }
            }
        }
    ...
    {
        $this->page = pathinfo((string) Request::get_page(), PATHINFO_FILENAME);
        $this->register($this->page);
        $this->register_pipeline('siteWide');
        $this->cat('injectAppTop');
    }
    public function register_pipeline($pipeline, &$parameters = null): void
    {
        $this->register($pipeline);
        $this->cat("{$pipeline}Start", $parameters);
    }
    public function set($action, $code, $callable): void
    {
