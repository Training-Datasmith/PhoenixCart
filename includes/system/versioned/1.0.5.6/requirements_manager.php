...
abstract class requirements_manager
{
    ...
    private ?array $matched_requirers = null;
    private $missing_abilities;
    ...
    public function act_on($field, $method, &$details = null)
    {
        ...
        if (method_exists($purveyor, $method)) {
            return $purveyor->$method($details);
        }
    ...
    public function process($requirements = [])
    {
        ...
        usort($purveyors, function ($a, $b): int {
            if (count(array_intersect($a::class::PROVIDES, $b::class::REQUIRES)) > 0) {
                return -1;
            }

            if (count(array_intersect($b::class::PROVIDES, $a::class::REQUIRES)) > 0) {
                return 1;
            }
    ...
    public function generate($details, $requirements): void
    {
    ...
    public function list_all_capabilities()
    {
        ...
        foreach ($this->objects as $module) {
            $capabilities = array_merge($capabilities, $module::class::PROVIDES);
        }
    ...
    public function find_providers($requirement, $exclude = ''): void
    {
        foreach ($this->objects as $object) {
    ...
    public function find_requirers($requirement, $exclude = ''): void
    {
        foreach ($this->objects as $object) {
            if (!empty($exclude) && $object instanceof $exclude) {
                continue;
            ...
            if (in_array($requirement, $object::REQUIRES)) {
                ...
                $this->matched_requirers[$requirement][] = $object::class;
            }
        }
    }
