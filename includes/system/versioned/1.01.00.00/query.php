...
class query
{
    ...
    public const CRITERIA_INTERSECTION = ' AND ';
    public const CRITERIA_UNION = ' OR ';
    ...
    public static function build_join($from, $to, $type = 'LEFT'): string
    {
        $to_alias = static::determine_alias($to);
        return " $type JOIN $to $to_alias USING (" . static::determine_id($to) . ')';
    }

    public static function add_criteria_tables(array $db_tables, $criteria): array
    {
    ...
    public static function add_search_criteria(string $sql, $key, $db_tables): string
    {
        ...
        return $sql . ('(' . implode(self::CRITERIA_UNION, $criteria) . ')');
    }
    ...
    public static function add_order_by($criteria)
    {
        ...
        foreach ($criteria as $db_table => $column_directions) {
            ...
            foreach ($column_directions as $column => $direction) {
                ...
                if (!empty($direction) && 'DESC' === strtoupper((string) $direction)) {
                    $sql .= ' DESC';
                }
    ...
    public static function build_specified_columns($db_tables): string
    {
        $sql = '';
    ...
    public static function build_columns(array $db_tables, $chain = []): string
    {
        $sql = '';
    ...
    protected static function _build_criteria($alias, $column_values): string
    {
        $sql = '';
        ...
        foreach ($column_values as $column => $value) {
            ...
            $sql .= is_array($value)
                  ? "$column IN (" . implode(', ', array_map(self::value(...), $value)) . ')'
