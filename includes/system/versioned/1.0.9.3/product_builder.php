...
EOSQL;
    ...
    public static function build_data_attributes($product, array $data = [])
    {
        ...
        $product->set('data_attributes', implode('', array_map(fn ($key, int|string $value) => ' ' . htmlspecialchars((string) $key) . '="' . htmlspecialchars((string) $value) . '"', array_keys($data), $data)));

        return $product->get('data_attributes');
    }

    public static function build_prid($uprid): int|false
    {
    ...
    public static function build_uprid($id, $params)
    {
        if (is_numeric($id)) {
            ...
            $first_bracket = strpos((string) $id, '{');
            ...
            foreach (explode('{', substr((string) $id, $first_bracket + 1)) as $attribute) {
