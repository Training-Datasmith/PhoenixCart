...
class email
{
     ...
     */
    public const IMAGE_TYPES = ['gif' => 'image/gif', 'jpg' => 'image/jpeg', 'jpeg' => 'image/jpeg', 'jpe' => 'image/jpeg', 'bmp' => 'image/bmp', 'png' => 'image/png', 'tif' => 'image/tiff', 'tiff' => 'image/tiff', 'swf' => 'application/x-shockwave-flash'];
    public const LINEFEEDS = ["\r\n", "\n", "\r"];
    protected $html;
    protected $text;
    protected $output;
    protected $html_text;
    ...
    public function __construct($headers = [])
    {
        ...
        $this->headers = array_merge($this->headers, array_filter(array_values($headers), fn($v) => !Text::is_empty($v)));
    }
    /**
     ...
     */
    public function get_file($filename): string|false
    {
    ...
    public function find_html_images($images_dir): void
    {
        ...
        preg_match_all('/"([^"]+\.(' . implode('|', $extensions) . '))"/Ui', (string) $this->html, $images);
        $html_images = [];
    ...
    public function add_message($email_text): void
    {
        // Build the text version
        $text = strip_tags(html_entity_decode((string) $email_text, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML5, $this->build_params['text_charset']));
    ...
    public function get_parameters($param_type, $value = null): array|false
    {
        $params = [];
        switch ($param_type) {
            case 'text':
    ...
    protected function _build_message(): ?\mime
    {
        ...
        switch (true) {
            ...
            case $text && $attachments:
                ...
                $message->add_subpart($this->text, $this->get_parameters('text'));
                return $message;
            case $html && !$attachments && !$html_images:
                if (Text::is_empty($this->html_text)) {
                    ...
                    $message->add_subpart($this->html_text, $this->get_parameters('text'));
                    $message->add_subpart($this->html, $this->get_parameters('html'));
                }
                break;
            case $html && !$attachments && $html_images:
                if (Text::is_empty($this->html_text)) {
                    ...
                    $message = new mime('', ['content_type' => 'multipart/alternative']);
                    $message->add_subpart($this->html_text, $this->get_parameters('text'));
                    $related = $message->add_subpart('', ['content_type' => 'multipart/related']);
                }
                $related->add_subpart($this->html, $this->get_parameters('html'));
                break;
            case $html && $attachments && !$html_images:
                $message = new mime('', ['content_type' => 'multipart/mixed']);
                if (Text::is_empty($this->html_text)) {
                    ...
                    $alt = $message->add_subpart('', ['content_type' => 'multipart/alternative']);
                    $alt->add_subpart($this->html_text, $this->get_parameters('text'));
                    $alt->add_subpart($this->html, $this->get_parameters('html'));
                }
                break;
            case $html && $attachments && $html_images:
                $message = new mime('', ['content_type' => 'multipart/mixed']);
                if (Text::is_empty($this->html_text)) {
                    ...
                    $alt = $message->add_subpart('', ['content_type' => 'multipart/alternative']);
                    $alt->add_subpart($this->html_text, $this->get_parameters('text'));
                    $related = $alt->add_subpart('', ['content_type' => 'multipart/related']);
                }
                $related->add_subpart($this->html, $this->get_parameters('html'));
                break;
        }
        if ($html && $html_images) {
            foreach ($this->html_images as $image) {
                $related->add_subpart($image['body'], $this->get_parameters('html_image', $image));
            }
        }
        return $message;
    }
     ...
     *                          - Default is iso-8859-1
     */
    public function build_message($params = []): bool
    {
        ...
        if (is_object($message)) {
            foreach ($this->attachments as $attachment) {
                $message->add_subpart($attachment['body'], $this->get_parameters('attachment', $attachment));
            }
     ...
     * @return mixed[]
     */
    public function normalize_headers($headers = []): array
    {
    ...
    {
        return preg_match('{[\x80-\xFF]}', (string) $s) ? '=?utf-8?B?' . base64_encode((string) $s) . '?=' : ($quoted ? '"' . addcslashes((string) $s, '"\\') . '"' : $s);
    }
    ...
    public function send($to_name, $to_addr, $from_name, $from_addr, $subject = '', $headers = [])
    {
        ...
        foreach (["\n", "\r"] as $line_ending) {
            foreach ([$to_name, $to_addr, $subject, $from_name, $from_addr] as $header_value) {
                if (str_contains((string) $header_value, $line_ending)) {
                    return false;
                }
            }
        }
        ...
        return mail((string) $to, (string) $this->ensure_encoding($subject), (string) $this->output, implode($this->lf, $headers), "-f{$from_addr}");
    }
    /**
     ...
     */
    public function get_rfc822($to_name, $to_addr, $from_name, $from_addr, $subject = '', $headers = []): string
    {
