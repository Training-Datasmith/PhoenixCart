...
class currencies
{
    ...
    // class methods
    public function format($number, $calculate_currency_value = true, $currency_type = null, $currency_value = null): string
    {
    ...
    public function format_raw($number, $calculate_currency_value = true, $currency_type = null, $currency_value = null): string
    {
        if (empty($currency_type)) {
            $currency_type = $_SESSION['currency'] ?? DEFAULT_CURRENCY;
        }
    ...
    public function set_currency(): void
    {
    ...
    {
        $location = strpos((string) $number, '.');
        // if there's a decimal point, increment the location to point after it
        if ((false === $location) || (strlen(substr((string) $number, ++$location)) <= $precision)) {
            // the number is already rounded sufficiently
            return $number;
        }

        $location += $precision;
        $next_digit = substr((string) $number, $location, 1);
        $number = substr((string) $number, 0, $location);
