...
class Shopping_Cart
{
    ...
    public function update_quantity($products_id, $quantity, $attributes = null): void
    {
        $products_id_string = Product::build_uprid($products_id, $attributes);
        $products_id = Product::build_prid($products_id_string);
        ...
        if (is_numeric($products_id) && isset($this->contents[$products_id_string]) && is_numeric($quantity)) {
            ...
            if (isset($_SESSION['customer_id'])) {
                ...
                EOSQL, (int) $quantity, (int) $_SESSION['customer_id'], $GLOBALS['db']->escape($products_id_string)));
            }
            foreach ($attributes ?? [] as $option => $value) {
                $this->contents[$products_id_string]['attributes'][$option] = $value;
                if (isset($_SESSION['customer_id'])) {
                    ...
                    EOSQL, (int) $value, (int) $_SESSION['customer_id'], $GLOBALS['db']->escape($products_id_string), (int) $option));
                }
            }
            // assign a temporary unique ID to the order contents to prevent hack attempts during the checkout procedure
            $this->cart_id = $this->generate_cart_id();
    ...
    public function attributes_price($product_id, array $attributes): int|float
    {
        $attributes_price = 0;
    ...
    public function generate_cart_id($length = 5): string
    {
        return Password::create_random($length, 'digits');
    }
    protected function has_virtual_attributes($product_id, $attributes): bool
    {
        foreach ($attributes as $value) {
            ...
            EOSQL, (int) $product_id, (int) $value));
