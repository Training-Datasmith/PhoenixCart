...
class product_loader extends capabilities_manager
{
    ...
    public const LISTENER_NAME = 'productCapabilities';
    public const CAPABILITIES = ['attributes' => 'Product::load_attributes', 'brand' => 'Product::load_brand', 'categories' => 'Product::load_categories', 'data_attributes' => 'Product::build_data_attributes', 'images' => 'Product::load_images', 'link' => 'Product::build_link', 'notify' => 'Product::load_notify', 'review_percentile' => 'Product::load_reviews', 'review_rating' => 'Product::load_reviews', 'reviews' => 'Product::load_reviews', 'tax_rate' => 'Product::load_tax_rate', 'translations' => 'Product::load_translations'];
    /**
     * @return array{name: mixed, values: non-empty-array<array{name: mixed, prefix: mixed, price: mixed, filename: mixed, maxdays: mixed, maxcount: mixed}>}[]
     */
    public static function load_attributes($product, $language_id = null): array
    {
        ...
        EOSQL, (int) ($language_id ?? $_SESSION['languages_id']), (int) $product->get('id')));
        $attributes = [];
        while ($attribute = $attributes_query->fetch_assoc()) {
            if (!isset($attributes[$attribute['options_id']])) {
                $attributes[$attribute['options_id']] = ['name' => $attribute['products_options_name'], 'values' => []];
            }
            $attributes[$attribute['options_id']]['values'][$attribute['options_values_id']] = ['name' => $attribute['products_options_values_name'], 'prefix' => $attribute['price_prefix'], 'price' => $attribute['options_values_price'], 'filename' => $attribute['products_attributes_filename'], 'maxdays' => $attribute['products_attributes_maxdays'], 'maxcount' => $attribute['products_attributes_maxcount']];
        }
    ...
    {
        if (isset($GLOBALS['brand']) && $GLOBALS['brand']->get_data('manufacturers_id') == $product->get('manufacturers_id')) {
     ...
     * @return mixed[]
     */
    public static function load_categories($product): array
    {
        $categories_query = $GLOBALS['db']->query(sprintf(<<<'EOSQL'
        SELECT categories_id
         FROM products_to_categories
         WHERE products_id = %d
        EOSQL, (int) $product->get('id')));
        $categories = [];
     ...
     * @return mixed[]
     */
    public static function load_images($product): array
    {
    ...
    public static function load_notifications($product)
    {
        ...
        EOSQL, (int) $product->get('id'), (int) $_SESSION['customer_id']));
     ...
     * @return mixed[][]
     */
    public static function load_reviews($product): array
    {
        ...
        EOSQL, $product->get('id'), $_SESSION['languages_id']));
        $sum = 0;
        $reviews = [];
        ...
        $product->set('review_percentile', number_format(count($reviews) ? 20 * $sum / count($reviews) : 0, 2));
    ...
    public static function load_tax_rate($product)
    {
        if (isset($GLOBALS['customer'])) {
            $tax_rate = Tax::get_rate($product->get('tax_class_id'), $GLOBALS['customer']->get('country_id'), $GLOBALS['customer']->get('zone_id'));
        } else {
     ...
     * @return mixed[][]
     */
    public static function load_translations($product): array
    {
        if (!$product->get('id')) {
            return [];
        }
