...
class class_index
{
    ...
    public function autoload(string $original_class): void
    {
        // convert camelCase class names to snake_case filenames
        $class = static::normalize_class_name($original_class);
    ...
    public function remove_prefix(&$file): bool
    {
        ...
        if (preg_match('{\Atemplates/[^/]+/includes/}', (string) $file, $matches)
    ...
    public function find_all_actions_under(string $directory): void
    {
    ...
    public function find_all_files_under(string $directory): void
    {
    ...
    public function find_all_hooks_under(string $directory): void
    {
        foreach (scandir($directory, SCANDIR_SORT_ASCENDING) as $site) {
            ...
            if (!is_dir($site_path = "$directory$site")) {
                continue;
            }
            foreach (scandir($site_path, SCANDIR_SORT_ASCENDING) as $group) {
                ...
                if (!is_dir($group_path = "$site_path/$group")) {
                    continue;
                }
    ...
    public function find_all_templates_under(string $directory): void
    {
    ...
    public function set(string $class, string $path): void
    {
        $this->files[static::normalize_class_name($class)] = $path;
    }

    public function set_translator($translator = null): void
    {
    ...
    {
        [$site, $group, $basename] = explode('/', substr($path, strlen($directory)), 3);
        return static::normalize_class_name(
            'hook_' . $site . '_' . $group . '_' . pathinfo($basename, PATHINFO_FILENAME)
        );
    }

    public static function normalize_class_name(string $original_class): string
    {
        return strtolower((string) preg_replace('{(?<!^)[A-Z]}', '_$0', $original_class));
    }
