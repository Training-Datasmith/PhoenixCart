...
{
    public const CODE = 'layout';
    public const DIRECTORY = DIR_FS_CATALOG . 'includes/modules/pi/';
    public const KEY = 'MODULE_LAYOUT_INSTALLED';
    public const TITLE = MODULE_CFG_MODULE_LAYOUT_TITLE;
    ...
    public const GET_HELP_LINK = 'https://phoenixcart.org/phoenixcartwiki/index.php?title=Layout';
    public const GET_ADDONS_LINKS = [ADDONS_FREE => 'https://phoenixcart.org/forum/app.php/addons/free/layout-26',
    ...
    public static function fix_installed_constant(&$installed_modules): bool
    {
        if (empty($_GET['page'])) {
            ...
            $page = basename(dirname((string) $GLOBALS['class_index']->get($_GET['module'])));
        } else {
            $page = $_GET['page'];
        }

        $key = static::GROUP_KEYS[pathinfo((string) $page, PATHINFO_FILENAME)] ?? null;
        ...
        $installed_modules = array_filter($installed_modules, function (array $v) use ($page): bool {
            if (isset($v['file']) && !isset($v['group'], $v['code'])) {
                $v['code'] = pathinfo((string) $v['file'], PATHINFO_FILENAME);
                $v['group'] = basename(dirname((string) $GLOBALS['class_index']->get($v['code'])));
            }
    ...
    public static function generate_modules()
    {
        if (empty($_GET['page'])) {
            ...
            $key = static::GROUP_KEYS[pathinfo((string) $_GET['page'], PATHINFO_FILENAME)]
                ?? null;
            $GLOBALS['cfg_modules']->set(static::CODE, 'key', $key);
            $GLOBALS['module_key'] = $key;
        ...
        if (!isset($_GET['list']) || ('new' !== $_GET['list'])) {
            ...
            array_splice($GLOBALS['table_definition']['columns'], 3, 0, [
              [
                ...
                'class' => 'text-end',
                'function' => fn ($row) => $row['display_group'],
              ],
            ]);
        }
    }
