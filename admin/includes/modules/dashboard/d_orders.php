...
{
    public const CONFIG_KEY_BASE = 'MODULE_ADMIN_DASHBOARD_ORDERS_';
...
EOSQL
        ...
        while ($order = $orders_query->fetch_assoc()) {
            $output .= '<tr>';
            $output .= '<td><a href="' . $GLOBALS['Admin']->link('orders.php', 'oID=' . (int)$order['orders_id'] . '&action=edit') . '">' . htmlspecialchars((string) $order['customers_name']) . '</a></td>';
            $output .= '<td>' . strip_tags((string) $order['order_total']) . '</td>';
