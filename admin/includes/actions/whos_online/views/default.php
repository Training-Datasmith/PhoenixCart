...
$table_definition = [
  'columns' => [
    [
      ...
      'function' => fn (&$row) => gmdate('H:i:s', time() - $row['time_entry']),
    ],
    [
      'name' => TABLE_HEADING_CUSTOMER_ID,
      'function' => fn (&$row) => $row['customer_id'],
    ],
    [
      'name' => TABLE_HEADING_FULL_NAME,
      'function' => fn (&$row) => $row['full_name'],
    ],
    [
      'name' => TABLE_HEADING_IP_ADDRESS,
      'function' => fn (&$row) => $row['ip_address'],
    ],
    [
      'name' => TABLE_HEADING_ENTRY_TIME,
      'class' => 'text-end',
      'function' => fn ($row) => date('H:i:s', $row['time_entry']),
    ],
    [
      'name' => TABLE_HEADING_LAST_CLICK,
      'class' => 'text-end',
      'function' => fn (&$row) => date('H:i:s', $row['time_last_click']),
    ],
    [
      'name' => TABLE_HEADING_LAST_PAGE_URL,
      'class' => 'text-end',
      'function' => fn (&$row) => preg_replace('{ceid=[A-Z0-9,-]+[&]*}i', '', (string) $row['last_page_url']),
    ],
    [
      'name' => TABLE_HEADING_ACTION,
      'class' => 'text-end',
      'function' => fn ($row) => (isset($row['info']->session_id) && ($row['session_id'] == $row['info']->session_id))
