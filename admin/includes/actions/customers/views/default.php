...
if (!Text::is_empty($_GET['search'] ?? '')) {
    ...
    $admin_hooks->set('customersListButtons', 'reset_keywords', fn () => $GLOBALS['Admin']->button(IMAGE_RESET, 'fas fa-angle-left', 'btn-light', $GLOBALS['Admin']->link('customers.php')));
}
...
$table_definition = [
  'columns' => [
    [
      ...
      'class' => 'col-1',
      'function' => fn (&$row) => $customer_data->get('id', $row),
    ],
    [
      'name' => TABLE_HEADING_NAME,
      'function' => fn (&$row) => $customer_data->get('sortable_name', $row),
    ],
    [
      'name' => TABLE_HEADING_ACCOUNT_CREATED,
      'class' => 'text-end',
      'function' => fn (&$row) => Date::abridge($customer_data->get('date_account_created', $row)),
    ],
    [
    ...
    [
      ...
      'class' => 'text-end',
      'function' => fn ($row) => (isset($row['info']->id) && ($row['info']->id === $customer_data->get('id', $row)))
...
$table_definition['function'] = function (array &$row) use ($customer_data, &$table_definition): void {
