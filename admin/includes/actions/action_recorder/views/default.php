...
$table_definition = [
  'columns' => [
    ...
    [
      ...
      'function' => fn (&$row) => htmlspecialchars((string) $row['user_name']) . ' [' . (int)$row['user_id'] . ']',
    ],
    [
      'name' => TABLE_HEADING_SUCCESS,
      'function' => fn (&$row) => ($row['success'] == '1')
    ...
    [
      ...
      'class' => 'text-end',
      'function' => fn (&$row) => $GLOBALS['date_time_formatter']->format((new Date($row['date_added']))->get_timestamp()),
    ],
    [
      'name' => TABLE_HEADING_ACTION,
      'class' => 'text-end',
      'function' => fn ($row) => (isset($row['info']->id) && ($row['info']->id === $row['id']))
