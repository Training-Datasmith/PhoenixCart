...
$table_definition = [
  'columns' => [
    [
      ...
      'function' => fn ($row) => $row['user_name'],
    ],
    [
      'name' => TABLE_HEADING_HTPASSWD,
      'class' => 'text-center',
      'function' => function (array $row) use ($apache_users, $is_iis): string {
          if ($is_iis) {
...
$table_definition['function'] = function (array &$row) use ($link, $action, &$table_definition): void {
    ...
    if (!isset($table_definition['info']) && (!isset($_GET['aID']) || ($_GET['aID'] == $row['id'])) && (!str_starts_with((string) $action, 'new'))) {
