...
if (!Text::is_empty($_GET['search'] ?? '')) {
    ...
    $admin_hooks->set('brandsListButtons', 'reset_keywords', fn () => $GLOBALS['Admin']->button(IMAGE_RESET, 'fas fa-angle-left', 'btn-light', $GLOBALS['Admin']->link('manufacturers.php')));
}
...
$table_definition = [
  'columns' => [
    [
      ...
      'is_heading' => true,
      'function' => fn ($row) => $row['manufacturers_name'],
    ],
    [
      'name' => TABLE_HEADING_ACTION,
      'class' => 'text-end',
      'function' => fn ($row) => (isset($row['info']->manufacturers_id) && ($row['manufacturers_id'] == $row['info']->manufacturers_id))
...
$table_definition['function'] = function (&$row) use (&$table_definition): void {
