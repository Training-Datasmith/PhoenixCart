...
$table_definition = [
  'columns' => [
    [
      ...
      'function' => fn (&$row) => $row['orders_id'],
    ],
    [
      'name' => TABLE_HEADING_CUSTOMERS,
      'function' => fn (&$row) => $row['customers_name'],
    ],
    [
      'name' => TABLE_HEADING_ORDER_TOTAL,
      'function' => fn (&$row) => strip_tags((string) $row['order_total']),
    ],
    [
      'name' => TABLE_HEADING_DATE_PURCHASED,
      'class' => 'text-end',
      'function' => fn (&$row) => $row['date_purchased'],
    ],
    [
      'name' => TABLE_HEADING_STATUS,
      'class' => 'text-end',
      'function' => fn (&$row) => $row['orders_status_name'],
    ],
    [
      'name' => TABLE_HEADING_ACTION,
      'class' => 'text-end',
      'function' => fn ($row) => '<a href="' . $row['onclick'] . '"><i class="fas fa-cogs me-2 text-dark"></i></a>'
...
$table_definition['function'] = function (array &$row) use ($link, &$table_definition): void {
