...
$mail_query = match ($_POST['customers_email_address']) {
    '***' => $db->query($customer_data->build_read(['name', 'email_address'], 'customers')),
    '**D' => $db->query($customer_data->build_read(['name', 'email_address'], 'customers', ['newsletter' => true])),
    default => $db->query($customer_data->build_read(
        ['name', 'email_address'],
        'customers',
