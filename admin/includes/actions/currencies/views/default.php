...
$table_definition = [
  'columns' => [
    ...
    [
      ...
      'function' => fn ($row) => $row['code'],
      ],
    [
      'name' => TABLE_HEADING_CURRENCY_VALUE,
      'function' => fn ($row) => number_format($row['value'], 8),
      ],
    [
      'name' => TABLE_HEADING_ACTION,
      'class' => 'text-end',
      'function' => fn ($row) => (isset($row['info']->currencies_id))
...
$table_definition['function'] = function (array &$row) use ($link, $action, &$table_definition): void {
    ...
    if (!isset($table_definition['info']) && (!isset($_GET['cID']) || ($_GET['cID'] == $row['currencies_id'])) && (!str_starts_with((string) $action, 'new'))) {
...
$admin_hooks->set('buttons', 'update_installed_currencies', fn () => (defined('MODULE_ADMIN_CURRENCIES_INSTALLED') && !Text::is_empty(MODULE_ADMIN_CURRENCIES_INSTALLED))
