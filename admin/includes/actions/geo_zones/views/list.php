...
$table_definition = [
  'columns' => [
    [
      ...
      'function' => fn ($row) => $row['countries_name'] ?? TEXT_ALL_COUNTRIES,
    ],
    [
      'name' => TABLE_HEADING_COUNTRY_ZONE,
      'function' => fn ($row) => $row['zone_name'] ?? PLEASE_SELECT,
    ],
    [
      'name' => TABLE_HEADING_ACTION,
      'class' => 'text-end',
      'function' => fn ($row) => (isset($row['info']->association_id) && ($row['info']->association_id === $row['association_id']))
...
$table_definition['function'] = function (array &$row) use (&$table_definition, $link): void {
