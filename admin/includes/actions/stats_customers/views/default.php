...
$table_definition = [
  'columns' => [
    ...
    [
      ...
      'function' => fn ($row) => $customer_data->get('name', $row),
      ],
    [
      'name' => TABLE_HEADING_TOTAL_PURCHASED,
      'class' => 'text-end',
      'function' => fn ($row) => $currencies->format($row['ordersum']),
      ],
    [
      'name' => TABLE_HEADING_ACTION,
      'class' => 'text-end',
      'function' => fn ($row) => '<a class="text-dark" href="' . $GLOBALS['link']->set_parameter('cID', $row['customers_id']) . '"><i class="fas fa-file-csv me-2"></i></a>'
...
$table_definition['function'] = function (array &$row): void {
    $row['css'] = '';
};
