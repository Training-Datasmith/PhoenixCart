...
$table_definition = [
  'columns' => [
    [
      ...
      'is_heading' => false,
      'function' => fn ($row) => $row['send_at'],
    ],
    [
      'name' => TABLE_HEADING_READY_TO_SEND,
      'class' => 'text-center',
      'is_heading' => false,
      'function' => fn ($row) => ($row['send_at'] < date('Y-m-d h:i:s'))
    ...
    [
      ...
      'is_heading' => false,
      'function' => fn ($row) => $row['fname'],
    ],
    [
      'name' => TABLE_HEADING_EMAIL,
      'is_heading' => false,
      'function' => fn ($row) => $row['email_address'],
    ],
    [
      'name' => TABLE_HEADING_SLUG,
      'is_heading' => false,
      'function' => fn ($row) => $row['slug'],
    ],
    [
      'name' => 'i18n',
      'is_heading' => false,
      'function' => function (array $row) use ($l) {
          foreach ($l as $data) {
              if ($data['id'] == $row['languages_id']) {
                  return $data['name'];
              }
          }
          return null;
    ...
    [
      ...
      'class' => 'text-end',
      'function' => fn ($row) => (isset($row['info']->id) && ($row['id'] == $row['info']->id))
