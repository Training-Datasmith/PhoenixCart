...
$table_definition = [
  'columns' => [
    [
      ...
      'is_heading' => true,
      'function' => fn ($row) => $row['title'],
    ],
    [
      'name' => TABLE_HEADING_MODULE,
      'function' => fn ($row) => $row['module'],
    ],
    [
      'name' => TABLE_HEADING_DATE_ADDED,
      'function' => fn ($row) => Date::abridge($row['date_added']),
    ],
    [
      'name' => TABLE_HEADING_SIZE,
      'function' => fn ($row) => number_format($row['content_length']) . ' bytes',
    ],
    [
      'name' => TABLE_HEADING_SENT,
      'class' => 'text-center',
      'function' => fn ($row) => ($row['status'] == '1')
    ...
    [
      ...
      'class' => 'text-end',
      'function' => fn ($row) => (isset($row['info']->orders_id))
