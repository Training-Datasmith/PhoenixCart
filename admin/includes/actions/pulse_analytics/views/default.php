...
if (!empty($conditions)) {
    $admin_hooks->set('eventListButtons', 'reset_keywords', fn () => $GLOBALS['Admin']->button(IMAGE_RESET, 'fas fa-angle-left', 'btn-light', $GLOBALS['Admin']->link('pulse_analytics.php')));
}
...
$table_definition = [
  'columns' => [
    [
      ...
      'function' => fn (&$row) => $row['event_type'],
    ],
    [
      'name' => TABLE_HEADING_CUSTOMER_ID,
      'function' => fn (&$row) => $row['customer_id'] ?? TEXT_INFO_NA,
    ],
    [
      'name' => TABLE_HEADING_PRODUCT_ID,
      'function' => fn (&$row) => $row['product_id'] ?? TEXT_INFO_NA,
    ],
    [
      'name' => TABLE_HEADING_CREATED_AT,
      'class' => 'text-end',
      'function' => fn (&$row) => $row['created_at'],
    ],
    [
      'name' => TABLE_HEADING_ACTION,
      'class' => 'text-end',
      'function' => fn ($row) => (isset($row['info']))
