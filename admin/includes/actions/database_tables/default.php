...
while ($table_info = $tables_query->fetch_assoc()) {
    $table_data[] = [
      htmlspecialchars((string) $table_info['Name']),
      htmlspecialchars((string) $table_info['Rows']),
      round(($table_info['Data_length'] + $table_info['Index_length']) / 1024 / 1024, 2) . 'M',
      htmlspecialchars((string) $table_info['Engine']),
      htmlspecialchars((string) $table_info['Collation']),
