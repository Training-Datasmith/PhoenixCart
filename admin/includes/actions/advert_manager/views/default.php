...
$table_definition = [
  'columns' => [
    [
      ...
      'function' => fn ($row) => $row['advert_title'],
    ],
    [
      'name' => TABLE_HEADING_GROUP,
      'function' => fn ($row) => $row['advert_group'],
    ],
    [
      'name' => TABLE_HEADING_SORT_ORDER,
      'function' => fn ($row) => $row['sort_order'] ?? 0,
    ],
    [
    ...
    [
      ...
      'class' => 'text-end',
      'function' => fn ($row) => (isset($row['info']->advert_id) && ($row['advert_id'] == $row['info']->advert_id))
