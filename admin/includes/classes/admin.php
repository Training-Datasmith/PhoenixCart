...
class Admin
{
    ...
    protected \Linker $catalog_linker;
    protected $processor;
    ...
    public function __construct($processor = null)
    {
        ...
        $this->set_processor($processor
          ?? pathinfo((string) Request::get_page(DIR_WS_ADMIN), PATHINFO_FILENAME));
    }
    ...
    public function set_processor(string $processor = null): void
    {
    ...
    {
        $pos_params = strpos((string) $url, '?', 0);
        if (false === $pos_params) {
            return $GLOBALS['Admin']->link($url);
        }

        parse_str(substr((string) $url, $pos_params + 1), $parameters);
        return $GLOBALS['Admin']->link(substr((string) $url, 0, $pos_params), $parameters);
    }
    ...
    public static function camel_case(string $snake_case): string
    {
    ...
    public static function catalog_image(string $image, ...$arguments)
    {
