...
class Config
{
    ...
    public static function select_multiple($selections, $key_values, $key_name = null): string
    {
        $key_name = static::name($key_name);
    ...
    public static function select_none_to_many($selections, $key_values, string $key_name): string
    {
        if (empty($key_values)) {
            $key_values = [];
        } elseif (!is_array($key_values)) {
            if (!str_contains($key_values, ';')) {
...
EOT;
    ...
    public static function select_one($select_options, $key_value, $key = ''): string
    {
        $string = '';
        $n = 1;
    ...
    public static function select_order_status(?string $order_status_id, $key = '')
    {
...
EOSQL
    ...
    public static function select_tax_class(?string $tax_class_id, $key = '')
    {
        ...
        return (new Select($name, array_merge(
            [['id' => '0', 'text' => TEXT_NONE]],
            Tax::fetch_classes()
        ), ['class' => 'form-select']))->set_selection($tax_class_id);
    }
