...
class tableBlock
{
    ...
    public function __construct($contents): string
    {
        $tableBox_string = '';

        for ($i = 0, $n = count($contents); $i < $n; $i++) {
            $tableBox_string .= '<tr';
            if (!Text::is_empty($this->table_row_parameters)) {
            ...
            if (isset($contents[$i][0]) && is_array($contents[$i][0])) {
                for ($x = 0, $y = count($contents[$i]); $x < $y; $x++) {
                    if (isset($contents[$i][$x]['text']) && !Text::is_empty($contents[$i][$x]['text'])) {
                        $tableBox_string .= '<td';
                        if (isset($contents[$i][$x]['class']) && !Text::is_empty($contents[$i][$x]['class'])) {
                            $tableBox_string .= ' class="' . $contents[$i][$x]['class'] . '"';
                        }
                        ...
                        $tableBox_string .= '>';
                        if (isset($contents[$i][$x]['form']) && !Text::is_empty($contents[$i][$x]['form'])) {
                ...
                $tableBox_string .= '<td';
                if (isset($contents[$i]['class']) && !Text::is_empty($contents[$i]['class'])) {
