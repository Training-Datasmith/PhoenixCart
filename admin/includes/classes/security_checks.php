...
class security_checks
{
    ...
    public const TYPES = ['info', 'warning', 'error'];

    protected $modules = [];
    protected $curl_handles = [];
    protected $curl_results = [];
    protected \CurlMultiHandle $multi_handle;
    ...
    public function __construct($extended = false)
    {
        ...
        usort($this->modules, fn (array $a, array $b) => strcasecmp((string) $a['title'], (string) $b['title']));
    }

    public function find_modules(string $directory): void
    {
     ...
     * @return CurlHandle
     */
    public function fetch_curl_handle(string $url, array $options = []): \CurlHandle|false
    {
    ...
    public function add_curl_handle($curl, $code): bool
    {
    ...
    {
        array_map(function ($m): void {
            $GLOBALS[$m]->close();
        }, array_filter(array_column($this->modules, 'class'), fn ($class) => is_callable([$GLOBALS[$class], 'close'])));
    }
