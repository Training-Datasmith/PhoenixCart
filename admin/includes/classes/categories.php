...
class Categories
{
    ...
    public static function generate_paths($ids, array $categories = []): array
    {
        foreach ($ids as $index => $id) {
            $categories[$index] = ('0' == $id)
                                ? [['id' => '0', 'text' => TEXT_TOP]]
                                : static::generate_path($id);
        }
     ...
     * @return mixed[]
     */
    public static function generate_path($id): array
    {
        $category_tree = & Guarantor::ensure_global('category_tree');
    ...
    public static function draw_breadcrumbs($ids): string
    {
        $tree = & Guarantor::ensure_global('category_tree');

        return implode('<br>', array_map(fn ($c) => implode('&nbsp;&gt;&nbsp;', array_map(fn ($id) => $tree->get($id, 'name'), array_merge(array_reverse($tree->get_ancestors($c)), [$c]))), $ids)) ?: TEXT_TOP;
    }

    public static function build_paths($ids): string
    {
        $display = new tree_display(Guarantor::ensure_global('category_tree'));

        return implode('<br>', array_map(fn ($id) => $display->buildBreadcrumb($id), $ids)) ?: TEXT_TOP;
    }

    public static function remove($category_id): void
    {
