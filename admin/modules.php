...
$modules_installed = (defined($module_key) && constant($module_key)) ? explode(';', (string) constant($module_key)) : [];
...
$table_definition = [
  'columns' => [
    ...
    [
      ...
      'class' => 'text-end',
      'function' => fn ($row) => isset($row['info']->code)
...
if (!isset($_GET['list']) || ('new' !== $_GET['list'])) {
    array_splice($table_definition['columns'], -1, 0, [
      [
        ...
        'class' => 'text-end',
        'function' => fn ($row) => is_numeric($row['sort_order']) ? $row['sort_order'] : '',
      ],
    ]);
}
  ...
  if (!isset($_GET['list'])) {
      ...
      if ($template_integration) {
          if (defined('TEMPLATE_BLOCK_GROUPS')) {
              $tbgroups = explode(';', (string) TEMPLATE_BLOCK_GROUPS);
