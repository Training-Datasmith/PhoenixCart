...
*/
$listing_split = new Split_Page_Results($listing_sql, $num_list, 'p.products_id');
...
<?php 
    ...
    <div class="card mb-2 card-body alert-filters">
      <ul class="nav d-flex gap-2">
        <li class="nav-item dropdown">
          <a href="#" class="btn btn-secondary dropdown-toggle" data-bs-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><?php 
    echo TEXT_SORT_BY;
    ?></a>
    ...
    foreach ($column_list as $i => $column) {
        if ($column_specifications[$column]['sortable']) {
            echo Split_Page_Results::create_sort_heading($_GET['sort'], $i + 1, $column_specifications[$column]['heading']);
        }
    }
    ?>
          </div>
        ...
        <?php 
    echo $GLOBALS['hooks']->cat('navItemFilters', $listing_parameters);
    ?>
      </ul>
    </div>
  ...
  <?php 
    ...
    while ($listing = $listing_query->fetch_assoc()) {
        ...
        $card = ['show_buttons' => 'True' === PRODUCT_LIST_BUTTONS];
