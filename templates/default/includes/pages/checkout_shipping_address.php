...
?>
  ...
  <div class="row">
    <div class="col-sm-7 mb-3">
      <p class="fs-5 fw-semibold mb-1"><?php 
echo TABLE_HEADING_ADDRESS_BOOK_ENTRIES;
?></p>
      
      <div><?php 
echo (new Form('select_address', $Linker->build('checkout_shipping_address.php'), 'post', ['class' => 'was-validated']))->hide('action', 'select');
?>
...
while ($address = $address_query->fetch_assoc()) {
    ...
    $tickable = new Tickable('address', ['value' => $address['address_book_id'], 'id' => $input_id, 'aria-describedby' => $input_id, 'class' => 'form-check-input'], 'radio');
    ?>
    ...
    ?></label></td>
              <td align="text-end">
                <div class="form-check">
                  <?php 
    echo $tickable->tick($address['address_book_id'] == $_SESSION['sendto']);
    ?>
...
echo SHIPPING_FA_ICON . $customer->make_address_label($_SESSION['sendto'], true, ' ', '<br>');
?></li>
        </ul>
      </div>
    </div>
...
if ($addresses_count < MAX_ADDRESS_BOOK_ENTRIES) {
    ...
    <p class="fs-5 fw-semibold mb-1"><?php 
    echo TABLE_HEADING_NEW_SHIPPING_ADDRESS;
    ?></p>
...
?>
  ...
  <div class="my-2">
    <?php 
echo new Button(IMAGE_BUTTON_BACK, 'fas fa-angle-left', 'btn-light', [], $Linker->build('checkout_shipping.php'));
?>
  </div>  
