...
if (!empty($_GET['action'])) {
    switch ($_GET['action']) {
        case 'dbCheck':
            try {
                $db = new Database(trim((string) $_GET['server']), trim((string) $_GET['username']), trim((string) $_GET['password']), '');
        ...
        case 'dbImport':
            try {
                $db_name = trim((string) $_GET['name']);
                $db = new Database(trim((string) $_GET['server']), trim((string) $_GET['username']), trim((string) $_GET['password']), $db_name);
