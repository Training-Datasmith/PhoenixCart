...
foreach ($_POST as $key => $value) {
    $log_params[$key] = stripslashes((string) $value);
}
...
try {
    ...
    if ($result == 'VERIFIED') {
        ...
        $check_query = $db->query('SELECT orders_status, customer_comments FROM orders WHERE orders_id = ' . $order_id . ' AND customers_id = ' . $customer_id);
        ...
        if ($check = $check_query->fetch_assoc()) {
            if ($check['orders_status'] == MODULE_PAYMENT_PAYPAL_STANDARD_PREPARE_ORDER_STATUS_ID) {
                ...
                $db->query('UPDATE orders SET orders_status = ' . (int)$order->info['order_status'] . ', last_modified = NOW() WHERE orders_id = ' . $order_id);

                if ('true' === DOWNLOAD_ENABLED) {
                    $downloads_query = $db->query('SELECT opd.orders_products_filename FROM orders o, orders_products op, orders_products_download opd WHERE o.orders_id = ' . $order_id . ' AND o.customers_id = ' . $customer_id . " AND o.orders_id = op.orders_id AND op.orders_products_id = opd.orders_products_id AND opd.orders_products_filename != ''");
                ...
                $db->query('DELETE FROM customers_basket WHERE customers_id = ' . $customer_id);
                $db->query('DELETE FROM customers_basket_attributes WHERE customers_id = ' . $customer_id);
            } else {
